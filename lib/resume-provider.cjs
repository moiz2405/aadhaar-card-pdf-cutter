...
// 18s per-provider timeout: with two providers racing plus cold-start
// overhead, this stays comfortably inside the 30s maxDuration.
const PROVIDER_TIMEOUT_MS = Number(process.env.RESUME_PROVIDER_TIMEOUT_MS || 18000);

function validateProviderUrl(rawUrl) {
  let parsed;
  try {
    parsed = new URL(rawUrl);
  } catch {
    throw new ResumeServiceError('provider_not_configured', 'The configured AI provider URL is invalid.', 503);
  }
  if (parsed.protocol !== 'https:') {
    throw new ResumeServiceError('provider_not_configured', 'The configured AI provider URL must use HTTPS.', 503);
  }
  return parsed.toString().replace(/\/$/, '');
}

function mapProviderHttpError(response) {
  // Surface misconfiguration and rate limits distinctly instead of a generic 502.
  if (response.status === 401 || response.status === 403) {
    return new ResumeServiceError(
      'provider_auth_error',
      'The AI provider rejected the configured API key. Please check the server configuration.',
      503,
    );
  }
  if (response.status === 429 || response.status === 503) {
    const retryAfter = response.headers && typeof response.headers.get === 'function'
      ...
      : '';
    const hint = retryAfter ? ` Please retry after ${retryAfter} seconds.` : ' Please try again shortly.';
    return new ResumeServiceError('provider_rate_limited', `The AI provider is rate limited.${hint}`, 429);
  }
  return new ResumeServiceError(
    'provider_error',
...
async function parseWithOpenAI(text, signal) {
  ...
  if (!response.ok) {
    throw mapProviderHttpError(response);
  }
...
async function parseWithChatCompletions({ text, baseUrl, apiKey, model, signal }) {
  ...
  const endpoint = `${validateProviderUrl(baseUrl)}/chat/completions`;
...
const PROVIDERS = {
  ...
  xkiro: (text, signal) => {
    if (!process.env.XKIRO_API_KEY) {
      throw new ResumeServiceError('provider_not_configured', 'xKiro is not configured.', 503);
    }
    return parseWithChatCompletions({
      text,
      baseUrl: process.env.XKIRO_BASE_URL || 'https://api.xkiro.com/v1',
      apiKey: process.env.XKIRO_API_KEY,
      model: process.env.XKIRO_MODEL || 'openai/gpt-5.6-luna',
      signal,
    });
  },
...
async function parseResumeText(text) {
  const configuredNames = process.env.RESUME_AI_PROVIDERS || process.env.RESUME_AI_PROVIDER || 'xkiro,opencode';
