...
function getClientId({ ip, forwardedFor }) {
  // X-Forwarded-For is spoofable: the client-controlled leftmost entry must
  // not be trusted. Vercel preserves the real client IP in the rightmost
  // position it appends, so take the last entry instead of the first.
  const entries = String(forwardedFor || '').split(',').map((part) => part.trim()).filter(Boolean);
  const forwarded = entries.length ? entries[entries.length - 1] : '';
  return forwarded || String(ip || 'unknown');
}

function getAllowedHosts() {
  const fromEnv = String(process.env.ALLOWED_HOSTS || '')
    .split(',')
...
function enforceOrigin(origin, host) {
  ...
  try {
    ...
    // Without an explicit allowlist, fall back to same-host comparison. The
    // Host header itself is client-controlled, so deployments that need real
    // CSRF protection should set ALLOWED_HOSTS.
