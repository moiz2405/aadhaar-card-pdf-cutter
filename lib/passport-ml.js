...
(function (root, factory) {
  ...
  // Cover-fit framing for a person box: scales so the person fills fillH of
  // the frame height (and at most fillW of the width), centers on the box,
  // then nudges down by headroom so the head keeps clearance. Never zooms
  // below cover scale, never zooms past maxZoom, pans clamped to cover.
  // Pure (mirrors coverDrawRaw) so it is unit-testable. All sizes in px.
  function framePerson(o) {
    const tW = o.tW; const tH = o.tH;
    const imgW = o.imgW; const imgH = o.imgH;
    const box = o.box;
    const fillH = o.fillH === undefined ? 0.88 : o.fillH;
    const fillW = o.fillW === undefined ? 0.92 : o.fillW;
    const headroom = o.headroom === undefined ? 0.04 : o.headroom;
    const maxZoom = o.maxZoom === undefined ? 3 : o.maxZoom;
    const s0 = Math.max(tW / imgW, tH / imgH);
    const pw = Math.max(1, box.x1 - box.x0);
    const ph = Math.max(1, box.y1 - box.y0);
    let s = Math.min((fillH * tH) / ph, (fillW * tW) / pw);
    s = Math.max(s0, Math.min(s, maxZoom * s0));
    const dw = imgW * s;
    const dh = imgH * s;
    ...
    const panX = Math.max(-mx, Math.min(mx, dw / 2 - cx * s));
    const panY = Math.max(-my, Math.min(my, dh / 2 - cy * s + headroom * tH));
    return { zoom: s / s0, panX, panY };
  }
